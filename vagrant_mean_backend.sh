...
echo "Creating Angular Server (Backend) Environtment"
sudo mkdir server
cd server
sudo npm init -y
...
recordRoutes.route(\"/employees\").get(function (req, res) {
 let db_connect = dbo.getDb(\"employeeDetails\");
 db_connect
...
recordRoutes.route(\"/employees/:id\").get(function (req, res) {
